...
interface VendorTermsDisplayProps {
  terms: string;
  className?: string;
  compact?: boolean;
}

export function VendorTermsDisplay({ terms, className, compact }: VendorTermsDisplayProps) {
  const sections = parseVendorTermsSections(terms);
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-primary/15 bg-primary/8 px-3 py-2.5">
        ...
        <div>
          <h3 className="text-sm font-semibold text-foreground">Vendor terms & conditions</h3>
          <p className="text-[10px] text-muted-foreground">Please read before applying</p>
        </div>
      </div>
